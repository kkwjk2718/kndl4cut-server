...
app.get('/admin', async (req, res) => {
  ...
  const qrCodes = await Promise.all(
    Array.from(activeFolders.entries()).map(async ([hash, folder]) => {
      const qrLink = `${domain}/photo/${hash}`;
      const photoPath = `${domain}/photos/${folder}/result.png`;

      // 사진 존재 여부 확인
      const fileExists = await fs.pathExists(path.join(rootFolder, folder, 'result.png'));

      return {
        hash,
        qrLink,
        photoUrl: fileExists ? photoPath : null, // 파일 존재 여부에 따라 URL 설정
...
app.get('/gallery', async (req, res) => {
  ...
  try {
    // activeFolders의 모든 사진 경로 추가
    for (const folderName of activeFolders.values()) {
      const photoPath = path.join(rootFolder, folderName, 'result.png');
      if (await fs.pathExists(photoPath)) {
        photos.push(`/photos/${folderName}/result.png`);
      }
    }
  } catch (err) {
...
app.listen(port, () => {
  ...
  console.log(`실시간 사진 전시: ${domain}/gallery`);
});
